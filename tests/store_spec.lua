...
describe("codeview.store", function()
  ...
  describe("the sync state", function()
    it("starts every comment as local", function()
      local store = new_store()
      local comment = add(store)
      assert.are.equal(0, comment.synced_at)
      assert.are.equal("", comment.review_id)
      assert.is_false(store_mod.is_synced(comment))
      assert.are.equal(1, #store:unsynced())
    end)

    it("marks one comment as sent", function()
      local store = new_store()
      local comment = add(store)
      local other = add(store, { body = "the other one" })
      local marked = assert(store:mark_synced(comment.id, { time = 1786527000, review_id = 42 }))

      assert.are.equal(1786527000, marked.synced_at)
      assert.are.equal("42", marked.review_id)
      assert.is_true(store_mod.is_synced(marked))
      assert.are.equal(comment.updated_at, marked.updated_at)
      assert.are.same({ other }, store:unsynced())
    end)

    it("reports a comment that the store does not hold", function()
      local store = new_store()
      local comment, err = store:mark_synced("no-such-id")
      assert.is_nil(comment)
      assert.are.equal("not_found", err.code)
    end)

    it("keeps the sync state in the file", function()
      local store = new_store()
      local comment = add(store)
      add(store, { body = "the other one" })
      assert(store:mark_synced(comment.id, { time = 1786527000, review_id = "42" }))
      assert(store:save())

      local back = assert(store_mod.load({ repo = store.repo, range = store.range }))
      assert.are.equal(1786527000, assert(back:get(comment.id)).synced_at)
      assert.are.equal("42", assert(back:get(comment.id)).review_id)
      assert.are.equal(1, #back:unsynced())
    end)

    it("clears the sync state of a comment that gets a new body", function()
      local store = new_store()
      local comment = add(store)
      assert(store:mark_synced(comment.id, { time = 1786527000, review_id = "42" }))
      local changed = assert(store:update(comment.id, { body = "a better note" }))

      assert.is_false(store_mod.is_synced(changed))
      -- The record of the review that took the first text stays.
      assert.are.equal("42", changed.review_id)
      assert.are.equal(1, #store:unsynced())
    end)

    it("keeps the sync state when the body does not change", function()
      local store = new_store()
      local comment = add(store)
      assert(store:mark_synced(comment.id, { time = 1786527000, review_id = "42" }))
      assert(store:update(comment.id, { body = comment.body, state = "resolved" }))

      assert.is_true(store_mod.is_synced(assert(store:get(comment.id))))
      assert.are.equal(0, #store:unsynced())
    end)

    it("writes an empty value for a comment that no submit sent", function()
      local store = new_store()
      add(store)
      local text = store_mod.encode(store)
      assert.is_truthy(text:find("- synced_at:\n", 1, true), text)
      assert.is_truthy(text:find("- review_id:\n", 1, true), text)
    end)
  end)
end)
