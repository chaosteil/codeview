...
describe("codeview.config", function()
  ...
  it("takes a boolean for the watcher of the repository", function()
    assert.is_true(config.defaults.auto_refresh)
    local cfg = assert(config.setup({ auto_refresh = false }))
    assert.is_false(cfg.auto_refresh)
    local bad, err = config.setup({ auto_refresh = "yes" })
    assert.is_nil(bad)
    assert.is_truthy(err:find("auto_refresh", 1, true), err)
  end)
